...
def load_server_credentials(file_path: str) -> grpc.ServerCredentials:
    try:
        with open(file_path + "/" + "server.crt", 'rb') as cf, open(file_path + "/" + "server.key", 'rb') as kf, open(
                file_path + "/" + "ca.crt", 'rb') as caf:
            certificate_chain = cf.read()
            private_key = kf.read()
            ca_cert = caf.read()
        return grpc.ssl_server_credentials(((private_key, certificate_chain),), root_certificates=ca_cert,
...
def load_client_credentials(file_path: str) -> grpc.ChannelCredentials:
    ...
    with open(file_path + "/" + "server.crt", 'rb') as scf, open(file_path + "/" + "client.crt", 'rb') as ccf, open(
            file_path + "/" + "client.key", 'rb') as kf:
        trusted_certs = scf.read()
        client_cert = ccf.read()
        client_key = kf.read()
    return grpc.ssl_channel_credentials(root_certificates=trusted_certs, private_key=client_key,
